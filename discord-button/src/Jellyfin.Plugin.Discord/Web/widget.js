(function () {
  ...
  // ── Config (cached) ────────────────────────────────────────────────────────
  var _cfg = null, _cfgLoading = false, _cfgQueue = [];
  ...
  function makeButton(cfg) {
    ...
    // Copie le style des boutons paper-icon-button-light de Jellyfin
    ...
    btn.addEventListener('mouseenter', function () { btn.style.backgroundColor = '#4752C4'; });
    btn.addEventListener('mouseleave', function () { btn.style.backgroundColor = '#5865F2'; });
  ...
  function inject(cfg) {
    ...
    // 1. Bouton SyncPlay — toujours présent dans le DOM (caché par .hide quand inactif)
    ...
    // 2. Fallback : conteneur .headerRight
    ...
    // 3. Fallback ultime : premier bouton du header skinHeader
    var headerBtn = document.querySelector('.skinHeader .headerButton');
    if (headerBtn && headerBtn.parentNode) {
      headerBtn.parentNode.insertBefore(btn, headerBtn);
    }
  }
  ...
  function check() {
    ...
    getConfig(function (cfg) {
      if (!cfg.ShowButton || !cfg.DiscordInviteUrl) return;
      inject(cfg);
      // Si pas encore injecté et qu'il reste des essais, on retente
      if (!document.getElementById('jf-discord-btn') && _tries < 20) {
        _tries++;
