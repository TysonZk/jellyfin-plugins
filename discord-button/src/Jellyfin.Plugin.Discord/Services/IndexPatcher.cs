...
public class IndexPatcher : IHostedService
{
    ...
    private void PatchIndex()
    {
        ...
        try
        {
            ...
            // L'URL est injectée directement — pas de href="#" qui déclenche la navigation
            var cfg    = Plugin.Instance?.Configuration;
            var url    = string.IsNullOrWhiteSpace(cfg?.DiscordInviteUrl) ? "javascript:void(0)" : cfg.DiscordInviteUrl;
            var target = (cfg?.OpenInNewTab ?? true) ? " target=\"_blank\"" : "";
            var tooltip = System.Net.WebUtility.HtmlEncode(cfg?.ButtonTooltip ?? "Discord");
            var show   = (cfg?.ShowButton ?? true);

            var btn    = show
                ? $"<a id=\"{BtnId}\" href=\"{System.Net.WebUtility.HtmlEncode(url)}\"{target} title=\"{tooltip}\" aria-label=\"{tooltip}\" style=\"{BtnStyle}\" rel=\"noopener\">{Svg}</a>"
                : "";
